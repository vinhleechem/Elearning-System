...
const AdminConversationDetail = () => {
  ...
  const fetchConversationDetail = async () => {
    ...
    setLoading(true);
    try {
      const conversationData = await conversationService.getConversationDetail(
        Number(id),
      );
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      if (response && response.data) {
        // Backend returns DESC, sort to ASC for display (oldest -> newest)
  ...
  const handleLockToggle = async () => {
    ...
    try {
      if (conversation.isLocked) {
        await conversationService.unlockConversation(
                  ...
                  <Stack direction="row" spacing={2} alignItems="flex-start">
                    ...
                    <Box flex={1}>
                      ...
                      <Typography variant="caption" color="text.secondary">
                        ...
                        {message.readAt &&
                          ` • Đã đọc lúc ${formatDate(message.readAt)}`}
                      </Typography>
                    </Box>
                    <IconButton
                      size="small"
